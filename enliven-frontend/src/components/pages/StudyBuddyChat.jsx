...
import {
  Send, Bot, User, BookOpen, ChevronDown, ChevronUp,
...
export default function StudyBuddyChat() {
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        // 1. Try activeCourseId from localStorage first
        let activeCourseId = localStorage.getItem("activeCourseId");

        // 2. If not set, fetch enrollments and use the first one
        if (!activeCourseId) {
          ...
          // Still fetch enrollments for the picker
  ...
  return (
    <>
      <style>{`
        ...
        .sb-inline-code { background:rgba(242,231,203,0.6); color:#C54F2D; border-radius:6px; padding:2px 6px; font-size:13px; font-family:monospace; font-weight:500; }
      ...
      <div className="flex flex-col h-[calc(100vh-5rem)] max-w-5xl mx-auto font-sans tracking-wide py-4 px-2 sm:px-6">
        <div className="bg-white border-2 border-cream rounded-[2rem] flex flex-col h-full shadow-sm overflow-hidden">
          ...
          {/* ── Header ── */}
          <div className="bg-white border-b-2 border-cream px-6 py-5 flex items-center justify-between shrink-0 z-10 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-[11px] font-bold uppercase tracking-widest text-foreground/50 mt-1 hover:text-red transition-colors"
                >
          ...
          {showPicker && (
            <div className="bg-white border-b-2 border-cream px-6 py-4 shrink-0">
              <p className="text-xs font-bold text-foreground/40 uppercase tracking-widest mb-3">Switch Course</p>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-sm font-bold border-2 transition-all ${
                      e.courseId === courseId
                        ...
                        : "bg-white text-foreground/70 border-cream hover:border-red/40 hover:text-red"
                    }`}
                  >
          ...
          <div className={`bg-cream/20 border-b-2 border-cream transition-all duration-300 overflow-hidden shrink-0 ${showCtx ? "max-h-96 opacity-100 py-6 px-6" : "max-h-0 opacity-0 py-0 px-6 border-b-0"}`}>
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                <div key={label} className={`rounded-2xl px-4 py-3 border-2 min-w-[120px] bg-white shadow-xs ${warn ? "border-red/30" : "border-cream"}`}>
                  <p className="text-foreground/50 mb-1 uppercase text-[10px] font-bold tracking-widest">{label}</p>
                  <p className={`font-bold text-sm ${warn ? "text-red" : "text-foreground"}`}>{value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* ── Messages ── */}
          <div className="flex-1 overflow-y-auto custom-scrollbar px-6 py-8 space-y-6 relative bg-cream/10">
            ...
            {fetching ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-6 text-center px-4 max-w-lg mx-auto">
                ...
                <div>
                  <h3 className="font-black text-foreground text-2xl tracking-tight">No course yet!</h3>
              ...
              <div className="flex gap-4 justify-start w-full">
                ...
                <div className="bg-white border-2 border-cream text-foreground rounded-[1.5rem] rounded-bl-md px-6 py-5 flex gap-2 items-center shadow-sm">
                  {[0, 0.2, 0.4].map((delay, i) => (
                    <span key={i} className="w-2.5 h-2.5 rounded-full bg-red/60 animate-bounce"
