...
export default function InitialAssessment() {
  ...
  const submitAssessment = async (finalAnswers) => {
    ...
    try {
      ...
      // ── 2. Generate roadmap (creates enrollment entry on backend) ─
      ...
      // ── 3. Persist courseId so studyBuddy utility can use it ─────
      if (rm?.courseId) {
        localStorage.setItem("activeCourseId", rm.courseId);
      }
      // Also cache the roadmap for legacy Sidebar fallback (harmless)
      localStorage.setItem("roadmap", JSON.stringify(rm));

      // ── 4. Update Study Buddy context with the newly known courseId ─
      await updateStudyBuddyContext({
        courseId:   rm?.courseId,
        event:      "roadmap_generated",
        domain,
        skillLevel,
        totalModules: rm?.topics?.length || 0,
      });
  ...
  const getCourseUrl = () => {
    ...
    const domain = String(roadmap.domain || "").toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-cream/30 font-sans p-6 overflow-hidden relative">
      {/* Decorative Blobs */}
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-yellow/20 rounded-full blur-[80px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-green/20 rounded-full blur-[80px] pointer-events-none" />
      ...
      <div className="w-full max-w-3xl bg-white rounded-[2rem] p-10 md:p-12 shadow-soft border border-cream/50 relative z-10 transition-all">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-3xl font-bold text-foreground">Technical Assessment</h2>
          </div>
          {q.difficulty && (
