...
import { BookOpen, Play, Flame } from "lucide-react";
...
export default function Dashboard() {
  ...
  const goToCourse = () => {
    ...
    if (roadmap?.domain && roadmap?.skillLevel) {
      ...
      navigate("/assessment"); // same fallback as sidebar
    }
  };
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="grid lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 grid sm:grid-cols-3 gap-6">
          ...
          <StatsCard title="Achievements" value={user.badges?.length || 0} icon={BookOpen} />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="bg-card rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-sm text-primary font-semibold uppercase">
                {continueLearning?.domain || user.domain}
              </p>
              <h3 className="text-xl font-bold">
                {continueLearning?.skillLevel || user.skillLevel}
              </h3>
          ...
          {continueLearning?.progress !== undefined && (
